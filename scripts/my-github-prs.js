...
function run() {
	...
	const requests = configs.map((config) => {
		const apiUrl = `${config.apiBase}/search/issues?q=author:${config.username}+is:pr+is:open&per_page=100`;
	...
	for (let i = 0; i < responses.length; i++) {
		...
		// Tag PRs with their source
		const taggedPrs = responseObj.items.map((/** @type {GithubIssue} */ item) => ({
			...item,
			_source: config.name,
		}));

		// Apply org filter by extracting owner from repository_url
		const filteredPrs = filterByOrg(taggedPrs, config.orgFilter, (item) => {
	...
	if (allPrs.length === 0) {
		return JSON.stringify({
			items: [{ title: "No open PRs found.", valid: false }],
		});
	}
	...
	const openPrs = allPrs.map((/** @type {GithubIssue} */ item) => {
